...
/* ─────────────────────────────────────────────────────────────────────────────
   MOCK DATA ARCHITECT
───────────────────────────────────────────────────────────────────────────── */
const ENRICH_DATA = (recs) => recs.map(rec => ({
  ...rec,
  estimated_time: "6-8 Months",
  market_demand: "High",
  job_roles: ["Senior " + rec.domain, "Architect", "Lead Specialist"],
  roadmap: [
    { phase: "Phase 1: Foundation", topics: ["Core Theory", "System Architecture", "Advanced Tools"] },
    { phase: "Phase 2: Integration", topics: ["API Design", "Data Modeling", "Cloud Deployment"] },
    { phase: "Phase 3: Optimization", topics: ["Performance Tuning", "Security Audits", "Scalability"] }
  ],
  priority_steps: [
    "Master " + (rec.missing_skills[0] || "Advanced Core"),
    "Build Project Evidence for " + (rec.missing_skills[1] || "Cloud Stack"),
...
const CareerRecommendations = () => {
  ...
  useEffect(() => {
    api.get('/careers/recommendations')
      .then((r) => {
        if (r.data.success && r.data.recommendations.length > 0) {
          setRecs(ENRICH_DATA(r.data.recommendations));
        } else {
          setRecs([]);
        }
      })
    ...
    <div className="animate-fade-up">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(380px, 1fr))', gap: '2rem' }}>
        {recs.length > 0 ? recs.map((rec, i) => (
          <motion.div key={rec.domain} whileHover={{ y: -5 }}>
            <GlassCard style={{ height: '100%', display: 'flex', flexDirection: 'column', position: 'relative' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem' }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  ...
                  <p style={{ fontSize: '2.25rem', fontWeight: 900, color: 'white' }}>{rec.confidence_score}%</p>
                </div>
              </div>
              ...
              <div style={{ marginBottom: '2.5rem' }}>
                <ProgressBar progress={rec.confidence_score} height={6} />
              </div>
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '1.5rem', marginBottom: '3rem' }}>
                <div>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                    {rec.matched_skills.slice(0, 3).map(s => <Badge key={s} variant="success">{s}</Badge>)}
                  </div>
                </div>
                <div>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                    {rec.missing_skills.slice(0, 3).map(s => <Badge key={s} variant="danger">{s}</Badge>)}
                  </div>
                </div>
              </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
             <GlassCard hover={false} style={{ padding: '2rem', background: 'rgba(255,255,255,0.01)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                   <MetricRow label="Estimate" value={selectedCareer?.estimated_time} icon={Clock} />
                   <MetricRow label="Demand" value={selectedCareer?.market_demand} icon={TrendingUp} />
                </div>
             </GlassCard>
             <Button variant="primary" fullWidth size="lg">Initialize Journey</Button>
          </div>
        </div>
      </Modal>
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '3rem' }}>
          <div style={{ textAlign: 'center' }}>
             <p style={{ fontSize: '3rem', fontWeight: 900, color: 'white', lineHeight: 1 }}>{selectedCareer?.confidence_score}%</p>
             <p style={{ fontSize: '0.7rem', fontWeight: 800, color: 'var(--primary)', textTransform: 'uppercase', marginTop: '0.5rem' }}>Readiness Score</p>
             <div style={{ marginTop: '1.5rem' }}>
                <ProgressBar progress={selectedCareer?.confidence_score} />
             </div>
          </div>
          <div>
